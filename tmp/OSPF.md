...
ABR(R2和R8)会产生一条7类缺省LSA，方便NSSA区域内部路由器访问其他区域引入的外部路由

多台ABR会同时产生7类缺省，NSSA区域内部IR路由器对缺省路由进行负载，存在次优风险
**次优的解决办法：**
1. 更改接口cost 
2. 更改下一跳权重值

**7类LSA的FA地址问题：**
1. FA地址和5类LSA的FA地址作用相同，都是用来做链路优化的
2. ==7类LSA的FA地址不会为空==
       如果场景和5类相同，那么FA地址会使外部路由下一跳地址，否则：
...
使用==CIDR==技术，将可以合并的路由条目汇总起来，减少LSDB中的LSA数量。

## 区域间汇总(只能在ABR进行汇总)

1. 在ABR上，可以针对区域内的1类2类LSA计算出的最优路由进行汇总，汇总后以3类LSA泛洪到其他直连区域
      备注：命令在哪个区域敲，汇总的就是哪个区域的叶子信息
2. 只要在LSDB中存在明细，那么汇总后的3类LSA就不会受到影响。当区域中的明细全部消失后，汇总后的3类LSA也会消失
3. 关于汇总后的cost问题，会使用明细路由中cost最大值作为汇总后的3类LSA的cost。
4. 如果存在多台ABR，那么每个ABR上都要进行汇总
5. 多ABR汇总后的3类LSA会在区域0泛洪给另外一台ABR，这是另外一台ABR不会计算这条3类LSA，为了避免环路

## 域外路由汇总(只能在ASBR进行汇总)

1. 针对type-1的外部路由，汇总后的开销值是所有被汇总路由中开销值最大的
2. 针对type-2的外部路由，汇总后的开销值是所有被汇总路由中开销值最大的+1
    备注：如果引入的明细既有type1又有type2，那么汇总后按照type2进行
3. 针对NSSA区域的ABR来说，他也是ASBR，也会做7转5的操作，如果需要对NSSA区域传递的外部路由进行汇总，
    需要再7转5的ABR上进行操作

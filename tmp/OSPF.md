...
1. 1、DD报文中第一个字段就是MTU，正常来说，OSPF邻居建立是要求2个接口的MTU值一致，否则无法建立邻接， 但是华为在实现邻居建立过程时，对于MTU的检测默认是忽略的。所以MTU值对于华为设备默认不影响。 （友商默认情况下是检查MTU的）
2. 2、可以认为使用命令开启MTU检测 [AR1-GigabitEtherneto/e/e]ospf mtu-enable MTu检测功能如果只在一段开启，另一端默认，那么不影响邻接建立 所以，两端都要开启